...
module Fees
  class CreatePastOnlineFees
    ...
    def create
      ...
      fts = nil
      remaining_amount = @fees_template.total_amount

      if create_params[:next_due_date].present?
        student_trans = FeesTransaction.where(student_id: create_params[:student_id])
        fts = student_trans.where('created_at > ?', create_params[:next_due_date]).order(:created_at)
        if fts.present?
          ref_ft = student_trans.where.not(id: fts.ids).order(:created_at).last
          remaining_amount = ref_ft.remaining_amount if ref_ft.present?
          new_ft = create_ft(create_params[:next_due_date])
          new_ft.remaining_amount = remaining_amount - new_ft.paid_amount
          new_ft.save

          remaining_amount = new_ft.remaining_amount
          fts.each do |nft|
            nft.remaining_amount = remaining_amount - nft.paid_amount
            nft.save
            remaining_amount = nft.remaining_amount
          end
        else
          new_ft = create_ft(create_params[:next_due_date])
        end
