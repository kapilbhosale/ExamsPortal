module Utils
  class _S3FolderUpload
    ...
    # s3_base_path - your path inside bucket
    # aws_key - Your key generated by AWS defaults to the env setting AWS_KEY_ID
    # aws_secret - The secret generated by AWS
    # include_folder - include the root folder on the path? (default: true)
    #
    # Examples
    #   => uploader = S3FolderUpload.new("some_route/test_folder", 'your-path-inside-bucket')
    #
    def initialize(folder_path, s3_base_path, aws_key = ENV['AWS_KEY_ID'], aws_secret = ENV['AWS_SECRET'], include_folder = true)
      Aws.config.update({
      ...
      @s3_bucket         = @connection.bucket(ENV['S3_BUCKET'])
